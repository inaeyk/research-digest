...
- campaign_state: M6_D_QUALIFIED_READY_TO_FREEZE
- current_substage: M6-D Library search and scientific connections qualified; local freeze pending
...
- current_qualification_state: M6-D repair round 1 deterministic qualification and fresh read-only repair Auditor PASS; local commit/tag freeze pending.
...
- next_permitted_action: stage inventory, inspect for secrets/runtime state, commit locally, and create annotated local tag `m6d-qualified`.
